...
__authors__ = ['Miao WANG','Jiaqian MA','Zeliang LIU','Yue CHEN']
__emails__  = ['miao.wang@student-cs.fr','jiaqian.ma@student-cs.fr','zeliang.liu@student-cs.fr','yue.chen@student-cs.fr']
...
class mySkipGram:
    def __init__(self, sentences, nEmbed=100, negativeRate=5, winSize = 5, minCount = 5, learning = 0.01):
        ...
        self.input_embedding = np.random.uniform(low=-0.5,high=0.5,size=(len(self.vocab_dict), self.nEmbed))

        # plain choice, all zeros
        # self.input_embedding = np.zeros([len(self.vocab_dict), self.nEmbed])
        self.output_weights = np.random.uniform(low=-0.5,high=0.5,size=(len(self.vocab_dict), self.nEmbed))
    ...
    def negative_sampling(self,word_tuple,alpha=0.75):
        ...
        negativeIds = np.random.choice(list(noise_dist_normalized.keys()), size=self.negativeRate, p=list(noise_dist_normalized.values()))
...
if __name__ == '__main__':
    # To not use scientific notations
    ...
    if not opts.test:
        ...
        sg = mySkipGram(sentences,nEmbed=150, negativeRate=10, winSize=9, minCount=6, learning=0.1)
        sg.train()
        sg.save(opts.model)
        ...
        for a,b,true_score in pairs:
            ...
            predict_similar.append(round(pred,4))
            true_similar.append(true_score)

        np.savetxt('results.txt', predict_similar,fmt='%.04f')
